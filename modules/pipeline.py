...
def set_path():
    ''' Получение пути к репозиторию проекта'''
    if __name__ == '__main__':
        # Если запускаем скрипт напрямую - от рабочего пути отрезаем 'modules/'
        path = os.environ.get('PROJECT_PATH', str(os.getcwd())[:-8])
    else:
        # Если вызываем через ДАГ - от рабочего пути отрезаем 'dags/'
        path = os.environ.get('PROJECT_PATH', str(os.getcwd())[:-5])
    # Результатом будет '~/airflow/'
    return path
